...
STORED_GRADS_DIR  = Path(f"./gradients/bio_{model_name}/")
SMALL_GRADS_DIR   = Path(f"./gradients/poison/bio_{model_name}/")
FORGET_OUTPUT     = Path(f"./selected_data/bio_{model_name}_avg.jsonl")
RETAIN_OUTPUT     = Path(f"./selected_data/bio_{model_name}_retain_avg.jsonl")
AVG_GRAD_PATH     = Path(f"./avg_gradients/bio_{model_name}_avg_grad.pt")
...
for d in ["./selected_data", "./avg_gradients", "./data/bio"]:
...
bio[bio["id"].isin(all_forget_ids)].to_parquet(
    f"./data/bio/{model_name}_grace_forget.parquet", index=False
)
...
# Exclude forget-set IDs from the pool
...
# Project gradients to be orthogonal to the average gradient
...
bio[bio["id"].isin(all_retain_ids)].to_parquet(
    f"./data/bio/{model_name}_grace_retain.parquet", index=False
)
